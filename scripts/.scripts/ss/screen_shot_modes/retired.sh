...
/usr/bin/maim -m 1 -u -i $(xdotool getactivewindow) | /usr/bin/convert - -resize 100% -define jpeg:dct-method=float -quality 90 ~/Pictures/${currentSession}_$(date '+%F_%H:%M:%S').jpeg &&
